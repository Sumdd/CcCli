...
namespace DataBaseUtil
{
	...
	public class Call_ServerListUtil
	{
		...
		{
			DataTable dt = new DataTable();
			string sqlstr = "select ServerIndex,ServerName,DomainName,ConnectTime,ServerIP,ServerPort,IsDefault from Call_Param left join Call_ServerList on Call_Param.ID=Call_ServerList.ParamID where Call_Param.P_Name='SipServer' and Call_Param.P_Value='SipServer' order by ServerIndex asc";
			dt = MySQL_Method.BindTable(sqlstr);
			return dt;
		}
